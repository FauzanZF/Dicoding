...
class MainActivity : AppCompatActivity() {
    ...
    private fun getListCities(): ArrayList<City> {
        ...
        val dataPulau = resources.getStringArray(R.array.data_pulau)
        val dataUmur = resources.getStringArray(R.array.data_umur)
        ...
        for (i in dataName.indices) {
            val city = City(dataName[i], dataDescription[i], dataPhoto.getResourceId(i, -1),
                dataPulau[i], dataUmur[i], dataTahun[i], dataIbukota[i])
            listCity.add(city)
        }
        return listCity
    }
